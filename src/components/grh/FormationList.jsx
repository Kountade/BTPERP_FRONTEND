// src/components/rh/FormationList.jsx
// Liste des formations des employés - Version corrigée

import React, { useState, useEffect } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { 
  GraduationCap, Plus, Edit, Trash2, Eye, Search,
...
function FormationList() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.log('📊 Formations chargées:', formationsRes.data?.length || 0);
  ...
  const filteredFormations = formations.filter(f => {
    const employeNom = (f.employe_nom || '').toLowerCase();
    const matchSearch = employeNom.includes(searchTerm.toLowerCase()) ||
                       (f.nom || '').toLowerCase().includes(searchTerm.toLowerCase());
    const matchEmploye = filterEmploye === 'all' || f.employe === parseInt(filterEmploye);
    const matchValide = filterValide === 'all' || 
                       (filterValide === 'valide' && f.valide) ||
                       (filterValide === 'non_valide' && !f.valide);
    return matchSearch && matchEmploye && matchValide;
  });
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('Token');
      await AxiosInstance.delete(`/formations/${selectedFormation.id}/`, {
        headers: { Authorization: `Token ${token}` }
      });
  ...
  const stats = {
    ...
    valides: formations.filter(f => f.valide).length,
    nonValides: formations.filter(f => !f.valide).length,
    totalCout: formations.reduce((acc, f) => acc + parseFloat(f.cout || 0), 0)
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-base-content/60">Chargement des formations...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 p-4 w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Link to="/formations/create" className="btn btn-primary gap-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="stat bg-base-100 rounded-lg shadow-sm p-4">
          ...
          <div className="stat-value text-2xl text-info">{stats.totalCout.toLocaleString()} €</div>
        </div>
      </div>
      ...
      {filteredFormations.length === 0 ? (
        <div className="text-center py-12 bg-base-100 rounded-lg shadow-sm">
          <GraduationCap className="w-16 h-16 text-base-content/20 mx-auto mb-4" />
          <h3 className="text-lg font-medium">Aucune formation trouvée</h3>
          <p className="text-base-content/60 text-sm mt-1">
            {searchTerm || filterEmploye !== 'all' || filterValide !== 'all'
              ? 'Aucune formation ne correspond à vos filtres'
              : 'Commencez par créer votre première formation'}
          </p>
          <Link to="/formations/create" className="btn btn-primary mt-4 gap-2">
            <Plus className="w-5 h-5" />
            Nouvelle formation
          </Link>
        </div>
        ...
        <div className="space-y-3">
          {filteredFormations.map((f) => {
            ...
            return (
              <div 
                key={f.id} 
                className={`bg-base-100 rounded-lg shadow-sm border ${f.valide ? 'border-success/30' : 'border-warning/30'} overflow-hidden transition-all`}
              >
                <div 
                ...
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className={`p-2 rounded-lg ${f.valide ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>
                      {f.valide ? <CheckCircle className="w-5 h-5" /> : <Clock className="w-5 h-5" />}
                    </div>
                    
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-semibold text-base truncate">
                          {f.employe_nom || 'Employé inconnu'}
                        </h3>
                        <span className={`badge ${f.valide ? 'badge-success' : 'badge-warning'} badge-sm`}>
                      ...
                      <div className="flex items-center gap-3 text-sm text-base-content/60 mt-0.5 flex-wrap">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {f.date_debut ? new Date(f.date_debut).toLocaleDateString('fr-FR') : 'N/A'}
                        </span>
                        <span className="flex items-center gap-1">
                          <DollarSign className="w-3 h-3" />
                          {parseFloat(f.cout || 0).toLocaleString()} €
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-base-200 p-4 bg-base-200/30">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        ...
                        <p className="text-sm">Employé: {f.employe_nom || 'N/A'}</p>
                        <p className="text-sm">Formation: {f.nom || 'N/A'}</p>
                        <p className="text-sm">Organisme: {f.organisme || 'N/A'}</p>
                      </div>
                      <div>
                        <h4 className="text-xs font-semibold uppercase text-base-content/40 mb-2 flex items-center gap-1">
                          <Calendar className="w-3 h-3" />
                        ...
                        <p className="text-sm">Début: {f.date_debut ? new Date(f.date_debut).toLocaleDateString('fr-FR') : 'N/A'}</p>
                        <p className="text-sm">Fin: {f.date_fin ? new Date(f.date_fin).toLocaleDateString('fr-FR') : 'N/A'}</p>
                        <p className="text-sm">Durée: {f.duree_heures || 0} heures</p>
                      </div>
                      <div>
                        <h4 className="text-xs font-semibold uppercase text-base-content/40 mb-2 flex items-center gap-1">
                          <DollarSign className="w-3 h-3" />
                        ...
                        <p className="text-sm">Coût: {parseFloat(f.cout || 0).toLocaleString()} €</p>
                        {f.certificat && (
                          <p className="text-sm">Certificat: {f.certificat}</p>
                        )}
                        {f.contrat_display && (
                          <p className="text-sm">Contrat: {f.contrat_display}</p>
                        )}
                      </div>
                    </div>
                  </div>
