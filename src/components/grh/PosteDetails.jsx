// src/components/rh/PosteDetails.jsx
...
function PosteDetails() {
  ...
  const CATEGORIES = {
    direction: { label: 'Direction', color: 'badge-error' },
    maitrise: { label: 'Maîtrise', color: 'badge-warning' },
    technicien: { label: 'Technicien', color: 'badge-info' },
    ouvrier: { label: 'Ouvrier', color: 'badge-success' },
    administratif: { label: 'Administratif', color: 'badge-primary' }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-base-content/60 text-sm mt-1">{error || 'Poste non trouvé'}</p>
          <button onClick={() => navigate('/postes')} className="btn btn-primary mt-4">
            Retour à la liste
          </button>
        </div>
      </div>
  ...
  const catConfig = CATEGORIES[poste.categorie] || CATEGORIES.technicien;
      ...
      <div className="bg-base-100 rounded-lg shadow-sm p-6 border border-base-200">
        <div className="flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            <h2 className="text-2xl font-bold">{poste.nom}</h2>
            <div className="flex items-center gap-3 mt-1 flex-wrap justify-center sm:justify-start">
              ...
              <span className={`badge ${catConfig.color} badge-lg gap-1`}>
                {catConfig.label}
              </span>
              {poste.nb_employes > 0 && (
                <span className="badge badge-info badge-lg gap-1">
                  <Users className="w-4 h-4" />
                  {poste.nb_employes} employés
                </span>
              )}
            </div>
          </div>
        ...
        <div className="bg-base-100 rounded-lg shadow-sm p-6 border border-base-200">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">
                <strong>Code:</strong> {poste.code || 'Non défini'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4 text-base-content/40" />
              <span className="text-sm">
                <strong>Catégorie:</strong> {catConfig.label}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Award className="w-4 h-4 text-base-content/40" />
              <span className="text-sm">
                <strong>Niveau:</strong> {poste.niveau || 'Non défini'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Shield className="w-4 h-4 text-base-content/40" />
              <span className="text-sm">
                <strong>Coefficient:</strong> {poste.coefficient || 'N/A'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4 text-base-content/40" />
              <span className="text-sm">
                <strong>Nombre d'employés:</strong> {poste.nb_employes || 0}
              </span>
            </div>
          </div>
        </div>
        ...
        {poste.competences_requises_detail && poste.competences_requises_detail.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {poste.competences_requises_detail.map((comp, idx) => (
              <span key={idx} className="badge badge-primary badge-lg gap-1">
                <CheckCircle className="w-3 h-3" />
                {comp.nom}
                {comp.categorie && (
                  <span className="text-xs text-primary-content/60 ml-1">
                    ({comp.categorie})
                  </span>
                )}
              </span>
          ...
          <p className="text-sm text-base-content/40">Aucune compétence requise pour ce poste</p>
        )}
      </div>

      {/* Employés occupant ce poste */}
      {poste.employes && poste.employes.length > 0 && (
        <div className="bg-base-100 rounded-lg shadow-sm p-6 border border-base-200">
          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
            <Users className="w-5 h-5 text-primary" />
            Employés ({poste.employes.length})
          </h3>
          <div className="space-y-3">
      ...
      <div className="bg-base-100 rounded-lg shadow-sm p-6 border border-base-200">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="stat bg-base-200/30 rounded-lg p-3">
            ...
            <div className="stat-value text-xl">{poste.nb_employes || 0}</div>
          </div>
          <div className="stat bg-base-200/30 rounded-lg p-3">
            <div className="stat-title text-xs">Actifs</div>
            <div className="stat-value text-xl text-success">
              {poste.employes?.filter(e => e.actif).length || 0}
            </div>
          </div>
          <div className="stat bg-base-200/30 rounded-lg p-3">
          ...
          <div className="stat bg-base-200/30 rounded-lg p-3">
            <div className="stat-title text-xs">Compétences requises</div>
            <div className="stat-value text-xl text-info">
              {poste.competences_requises_detail?.length || 0}
            </div>
          </div>
        </div>
      </div>
